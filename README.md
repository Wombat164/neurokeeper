...
> Status: **0.11.2 (alpha).** Licensed **MIT** (see [`LICENSE`](LICENSE)).
...
The core deliberately refuses domain-specific content, which would leave "get it merged upstream" as the
...
```
A name colliding with a built-in is refused rather than silently preferred either way, and a directory
...
- uses: Wombat164/neurokeeper@v0.11.2
