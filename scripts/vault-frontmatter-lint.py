...
def main():
    ...
    unknown = Counter()
    # per-finding + per-offvocab-value file lists, so --json is actionable without a second grep pass (issue #3)
    files = {k: [] for k in ("no_fm", "parse_err", "missing_note_type", "missing_sphere", "unknown_fields")}
